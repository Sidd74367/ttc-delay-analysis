...
All statistical tests (chi-squared goodness-of-fit, Mann–Whitney U) are implemented from scratch with `math` / `numpy`, so no `scipy` dependency is required. Hex does not support `scipy`.
